...
void LC_FREQCHANGE(unsigned int coarse, unsigned int mid, unsigned int fine){
...
void LC_monotonic_mid_fine(unsigned int LC_code, unsigned int mid0, unsigned int coarse0, unsigned int *mid, unsigned int *fine) {
	unsigned int before = LC_code;
	LC_code = LC_code % coarse0;
	*mid = LC_code / mid0 + 1;
	*fine = LC_code % mid0 + 1;
	
	printf("LC_code: %d code: %d mid0 %d coarse0 %d mid %d fine %d\n", before, LC_code, mid0, coarse0, *mid, *fine);
}

void LC_monotonic(unsigned int LC_code){
	/*
	unsigned int i;
	unsigned int coarse, mid, fine;
			
	unsigned int code_index = coarse_codes_length - 1;
	
	for (i = 0; i < coarse_codes_length; ++i) {
		if (LC_code < cumulative_coarse0_codes[i]) {
			code_index = i;
			break;
		}
	}
	...
	LC_monotonic_mid_fine(LC_code - (code_index > 0 ? cumulative_coarse0_codes[code_index - 1] : 0), mid0_codes[code_index], coarse0_codes[code_index], &mid, &fine);
	*/
