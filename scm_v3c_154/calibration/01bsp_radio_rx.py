...
while True:
    ...
    if rawFrame[-1:]==[0xff] and len(rawFrame)>=8:

        (rxpk_len,rxpk_num,rxpk_rssi,rxpk_crc,rxpk_coarse,rxpk_mid,rxpk_fine) = \
            struct.unpack('>BBbBBBB', b''.join([bytes([b]) for b in rawFrame[-8:-1]]))
        print('len={0:<3} num={1:<3} rssi={2:<4} crc={3} coarse={4} mid={5} fine={6}'.format(
            rxpk_len,
            rxpk_num,
            rxpk_rssi,
            rxpk_crc,
